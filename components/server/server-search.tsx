...
function ServerSearch({
    ...
    const onClick = ({ id , type } : { id : string, type : "channel" | "member" }) => {
        ...
        if(type === "member") {
            return router.push(`/severs/${params?.serverId}/conversations/${id}`)
        }

        if (type === "channel") {
            return router.push(`/servers/${params?.sersverId}/channels/${id}`)
        }
  ...
  return (
    <>
        ...
        className="group px-2 py-2 rounded-md flex
         items-center gap-x-2 w-full hover:bg-zinc-700/10 dark:hover:bg-zinc-700/50
         transition">
